...
if __name__ == "__main__":
    ...
    def _pb(it, **kw):  # pragma: no cover
        return tqdm(it, **kw)
    # ---------- parameters ----------
    nu     = 1e-1           # viscosity
    L      = 2*np.pi        # domain length
    nx     = 201           # grid points (including endpoints)
    T      = 5            # final time
    nt     = 101            # number of output frames
    Border = 8             # basis "border" → degree = Border + 1
    progress = True

    # ---------- grid, operator, IC ----------
    x  = np.linspace(0.0, L, nx)
    dx = x[1] - x[0]
    bf = bspf1d.from_grid(degree=Border+1, x=x)

    # Initial condition that is compatible with Neumann BC (u_x=0 at ends)
    u0 = np.sin(x)  # sin' is zero at 0 and 2π; feel free to change IC
    ...
    # ---------- RHS with Neumann BC enforced inside bfpsm ----------
    def burgers_rhs(t: float, u: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
        # Enforce homogeneous Neumann in the derivative operators
        du_dx, d2u_dx2, f_spline = bf.differentiate_1_2(u, neumann_bc=(-1.0, -1.0))
        rhs = nu * d2u_dx2          # u_t + u u_x = ν u_xx
        # Note: du_dx is ~0 at endpoints by construction, so rhs endpoints are ν u_xx
        return rhs, f_spline

    # ---------- choose RK4 timestep ----------
    umax0 = float(np.max(np.abs(u0))) + 1e-14
    if nu > 0:
    ...
    dt      = T / max(n_steps, 1)
    print(f"[RK4] dt={dt:.3e}, steps={n_steps} (adv={dt_adv:.3e}, diff={dt_diff:.3e})")

    # ---------- RK4 time march ----------
    u = u0.copy()
    t_curr = 0.0
    out_idx = 1
    for _ in _pb(range(n_steps), total=n_steps, disable=not progress):
        u_old = u.copy()

        k1, _ = burgers_rhs(t_curr, u)
        k2, _ = burgers_rhs(t_curr + 0.5*dt, u + 0.5*dt*k1)
        k3, _ = burgers_rhs(t_curr + 0.5*dt, u + 0.5*dt*k2)
        k4, _ = burgers_rhs(t_curr + dt,        u + dt*k3)

        u = u_old + (dt/6.0)*(k1 + 2*k2 + 2*k3 + k4)
        # u = apply_filter_dct(u)
        t_curr += dt

        # save outputs at requested times
        while out_idx < nt and t_curr >= t_out[out_idx] - 1e-12:
            U[out_idx] = u
            out_idx += 1
    ...
    try:
        ...
        plt.title("1D Burgers with Neumann BC (BSLF)")
