...
The root-level [`bspf1d.py`](/Users/moulin/Library/CloudStorage/Dropbox/workspace/bspf/bspf1d.py) remains in the repository as the **legacy compatibility implementation**.
...
- the lowercase alias `bspf.bspf1d` is still available
...
4. New internal development should happen under `src/bspf/`, not by adding new features directly to the monolithic legacy file.

## Migration Guidance

Preferred migration path for downstream users:

1. Replace `from bspf1d import bspf1d` with `from bspf import BSPF1D`.
2. Replace `from bspf1d import PiecewiseBSPF1D` with `from bspf import PiecewiseBSPF1D`.
3. Keep the old imports only where legacy notebooks or scripts require zero code churn.

## Repository Policy

- [`bspf1d.py`](/Users/moulin/Library/CloudStorage/Dropbox/workspace/bspf/bspf1d.py) is treated as a frozen legacy reference unless a bug fix is required for compatibility.
- `src/bspf/` is where the package architecture, tests, and new refactors should continue.
