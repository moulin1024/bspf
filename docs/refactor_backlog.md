...
1. Add a package scaffold under `src/bspf` that re-exports the legacy implementation.
...
- `src/bspf` package scaffold
- minimal test scaffold
- migration backlog document

Exit criteria:

- `from bspf import BSPF1D, Grid1D, PiecewiseBSPF1D` works
...
1. Move array aliases and shared typing helpers into `bspf/types.py`.
2. Move `Grid1D` into `bspf/grid.py`.
3. Move knot generation helpers into `bspf/knots.py`.
...
1. Move `BSplineBasis1D` into `bspf/basis.py`.
2. Move `EndpointOps1D` into `bspf/boundary.py`.
...
1. Move `ResidualCorrection` into `bspf/correction.py`.
2. Create `bspf/kkt.py` for KKT assembly, LU caching, and coefficient solve helpers.
...
1. Create a proper `BSPF1D` implementation in `bspf/operators/bspf1d.py`.
...
1. Move differentiation logic into `bspf/ops/differentiation.py`.
2. Move integration and antiderivative logic into `bspf/ops/integration.py`.
3. Move interpolation logic into `bspf/ops/interpolation.py`.
...
1. Move `PiecewiseBSPF1D` into `bspf/operators/piecewise.py`.
