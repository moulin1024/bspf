...
def taylor_green_velocity(x, y, z):
    """
    ...
    u = to_nz_ny_nx(np.sin(X) * np.cos(Y) * np.cos(Z))
    v = to_nz_ny_nx(-np.cos(X) * np.sin(Y) * np.cos(Z))
    w = to_nz_ny_nx(np.zeros_like(X))
    return u, v, w

def taylor_green_gradient_tensor(x, y, z):
    """
    Analytical gradient tensor for Taylor–Green vortex:
      ∇u = [∂u/∂x  ∂u/∂y  ∂u/∂z]   [cos(x)cos(y)cos(z)  -sin(x)sin(y)cos(z)  -sin(x)cos(y)sin(z)]
           [∂v/∂x  ∂v/∂y  ∂v/∂z] = [sin(x)sin(y)cos(z)  -cos(x)cos(y)cos(z)   cos(x)sin(y)sin(z)]
           [∂w/∂x  ∂w/∂y  ∂w/∂z]   [0                   0                    0                 ]
    ...
    # Derivatives of u
    du_dx = to_nz_ny_nx(np.cos(X) * np.cos(Y) * np.cos(Z))
    du_dy = to_nz_ny_nx(-np.sin(X) * np.sin(Y) * np.cos(Z))
    du_dz = to_nz_ny_nx(-np.sin(X) * np.cos(Y) * np.sin(Z))
    
    # Derivatives of v
    dv_dx = to_nz_ny_nx(np.sin(X) * np.sin(Y) * np.cos(Z))
    dv_dy = to_nz_ny_nx(-np.cos(X) * np.cos(Y) * np.cos(Z))
    dv_dz = to_nz_ny_nx(np.cos(X) * np.sin(Y) * np.sin(Z))
    
    # Derivatives of w (all zero)
    dw_dx = to_nz_ny_nx(np.zeros_like(X))
    dw_dy = to_nz_ny_nx(np.zeros_like(X))
    dw_dz = to_nz_ny_nx(np.zeros_like(X))
...
def build_facade(nx, ny, nz, degx, degy, degz, ordx, ordy, ordz, use_gpu, domain=(0.0, 2.0*np.pi)):
    ...
    return bspf3d.from_grids(
        ...
        order_x=ordx, order_y=ordy, order_z=ordz,
        correction="spectral",
        use_gpu=use_gpu,
    ), x, y, z

def prepare_plans_3(b3d, *, ox, oy, oz, lamx, lamy, lamz, neumann, uniform_bc):
    # For uniform BC we just use zeros as a single RHS vector unless user wants custom bc
    bc = 0.0 if uniform_bc else None
    t0 = time.perf_counter()
    px = b3d.make_plan_dx(order=ox, lam=lamx, neumann=neumann, uniform_bc=uniform_bc, bc=bc)
    py = b3d.make_plan_dy(order=oy, lam=lamy, neumann=neumann, uniform_bc=uniform_bc, bc=bc)
    pz = b3d.make_plan_dz(order=oz, lam=lamz, neumann=neumann, uniform_bc=uniform_bc, bc=bc)
    if b3d.use_gpu:
        sync_gpu()
...
def main():
    ...
    px_cpu, py_cpu, pz_cpu, prep_cpu = prepare_plans_3(
        ...
        neumann=False, uniform_bc=False
    )
    ...
    ug = cp.sin(Xg) * cp.cos(Yg) * cp.cos(Zg)
    vg = -cp.cos(Xg) * cp.sin(Yg) * cp.cos(Zg)
    wg = cp.zeros_like(Xg)
    ...
    eval_gpu, per_derivative_gpu = eval_gradient_tensor(
        ...
        neumann=False, flux_dict=flux_dict_gpu, use_gpu=True
    )
