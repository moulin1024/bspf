...
from bspf.solver import create_heat_rhs_2d
...
def main():
    ...
    # Create RHS function using modular heat solver
    rhs_func = create_heat_rhs_2d(op, nu, flux_x=(0.0, 0.0), flux_y=(0.0, 0.0))

    # Also make first-derivative plans for flux checks (they enforce 0 flux too)
    ...
    # Initialize time stepper with RK23 method
    state = TimeStepperState(U0.copy(), t_init=0.0, dt=dt, method='rk23')
    
    t0 = time.time()
    for step in _iter(range(1, nsteps + 1), total=nsteps):
        # Use modular time stepper with RK23
        _ = time_step(state, dt, rhs_func, method='rk23')
        U = state.get_current()
