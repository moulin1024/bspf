...
from bfpsm1d import bfpsm1d  # 1D facade with cached LU and basis blocks
...
class _AxisPlan:
    ...
    def __post_init__(self):
        ...
        # Precompute LU of KKT (bfpsm1d caches per lam internally; we hold a handle)
...
class bfpsm2d:
    """
    Vectorized 2D facade composed from two bfpsm1d models.
    ...
    x: Array           # (nx,)
    y: Array           # (ny,)
    x_model: bfpsm1d   # acts along axis=1 (x)
    y_model: bfpsm1d   # acts along axis=0 (y)
    ...
    @staticmethod
    def _diff_axis(F: Array, model: bfpsm1d, *, lam: float, k: int, axis: int, return_spline: bool):
        """
        Vectorized derivative of order k along `axis` using a single bfpsm1d model.
        """
    ...
    def _diff_axis_neumann(
        ...
        *,
        ...
        left_flux  = bfpsm2d._broadcast_flux(flux[0], batch)
        right_flux = bfpsm2d._broadcast_flux(flux[1], batch)
