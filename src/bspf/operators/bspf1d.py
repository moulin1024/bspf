...
The constructor and core state assembly now live in the package and compose the
extracted grid, basis, boundary, correction, and KKT helpers. The operation
methods are still borrowed from the legacy implementation for now and will be
split into dedicated modules in Phase 5.
"""
    ...
    def __init__(
        ...
        *,
        ...
        if self.use_gpu and _HAS_CUPY:
            ...
            # Preserve the legacy CPU layout and preallocation strategy so the
            # borrowed operation methods keep matching existing behavior.
