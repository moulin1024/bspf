...
def create_endpoint_clustering_mapping(domain, alpha=2.0, method='tanh'):
    """
    Create a mapping with clustering only at endpoints (no middle clustering).
    
    This is useful for boundary layer problems where you need high resolution
    near the boundaries but uniform resolution in the middle.
    
    Parameters
    ----------
    domain : tuple
        Tuple (a, b) defining the domain interval
    alpha : float, optional
        Clustering strength. Larger values give stronger clustering (default: 2.0)
    method : str, optional
        Method to use: 'tanh' (recommended) or 'sigmoid' (default: 'tanh')
    ...
    dexpr_v : sympy.Basic
        SymPy expression for dφ/dt
    centers : list
        List of sigmoid centers (for display purposes)
    
    Examples
    --------
    >>> domain = [0, 2*np.pi]
    >>> phi, dphi, centers = create_endpoint_clustering_mapping(domain, alpha=2.0)
    """
    domain = validate_domain(domain)
    a, b = domain
    L = b - a
    ...
    if method == 'tanh':
        # Tanh-based endpoint clustering: maps [a,b] -> [0,1]
        # This is the recommended method as it's simpler and more intuitive
        u = 2.0 * (t - a) / L - 1.0  # Normalize to [-1, 1]
        xi_raw = sp.tanh(alpha * u) / sp.tanh(alpha)  # Apply tanh clustering
        phi = (xi_raw + 1.0) / 2.0  # Map to [0, 1]
        dphi = sp.diff(phi, t)
        # Approximate centers for display (not used in computation)
        centers = [a + L/4, a + 3*L/4]
        
    elif method == 'sigmoid':
        # Sigmoid-based endpoint clustering using two segments
        # Left segment: clusters near a
        # Right segment: clusters near b
        # Middle: uniform (height = 0)
        p_vals = [0.5]  # One connection point in the middle
        k_vals = [alpha, alpha]  # Sharpness for left and right
        h_vals = [0.5, 0.5]  # Heights for left and right segments
        m_val = 0.1  # Baseline slope
        phi, dphi, centers = build_expr_via_connections_with_values(
            p_vals, k_vals, h_vals, m_val, normalize=True, domain=domain
        )
    else:
        raise ValueError(f"Unknown method: {method}. Use 'tanh' or 'sigmoid'.")
    
    return phi, dphi, centers
